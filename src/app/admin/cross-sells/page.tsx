...
export default async function CrossSellsAdminPage() {
  ...
  const [{ data: categories }, { data: complements }] = await Promise.all([
    ...
    supabase
      ...
      .select("id, category:categories!category_complements_category_id_fkey(*), complement:categories!category_complements_complement_category_id_fkey(*)")
      .order("created_at", { ascending: false }),
  ]);

  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <AdminNav active="cross-sells" />

      <h1 className="mb-1 text-2xl font-bold tracking-tight">Cross-sell Pairings</h1>
      <p className="mb-6 text-sm text-neutral-500">
        When a shopper views a product in the first category, they&apos;ll see cross-sells
        pulled from the second category. E.g. Cameras → Lenses, Batteries, Chargers.
        There&apos;s no order history yet to learn this automatically, so it&apos;s curated here.
      </p>
        ...
        {!complements || complements.length === 0 ? (
          ...
          <div className="flex flex-col divide-y divide-neutral-200 rounded-lg border border-neutral-200 bg-white">
            {(complements as unknown as { id: string; category: Category; complement: Category }[]).map(
              (row) => (
                <ComplementRow key={row.id} id={row.id} category={row.category} complement={row.complement} />
              )
            )}
          </div>
        )}
...
function ComplementRow({
  ...
  return (
    <form action={removeCategoryComplement} className="flex items-center justify-between p-3">
