...
/** Deliberately NOT wrapped in RevealOnScroll (framer-motion's scroll-fade
 *  wrapper) — that starts elements at opacity:0 and only reveals them once
 *  client JS detects the intersection. Fine for below-the-fold decorative
 *  content, but this is above-the-fold navigation; if hydration is slow or
 *  fails on a given device, the whole category row would stay invisible
 *  forever even though it's right there in the DOM. Critical nav should
 *  never depend on JS succeeding just to be visible. */
